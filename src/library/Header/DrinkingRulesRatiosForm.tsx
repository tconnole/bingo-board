    ...
    return (
        <div style={{width: '31%', aspectRatio: '1', padding: '0.25rem'}}>
            <div style={{width: '100%', height: '100%', backgroundColor: DrinkRuleColor(props.rule), display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', borderRadius: '0.25rem'}}>
                <Typography sx={{fontSize: '3rem', marginBottom: '1.5rem', textShadow: '1px 1px 2px black'}}>{DrinkRuleIcon(props.rule)}</Typography>
                ...
                <IconButton onClick={subtractClick} sx={{position: 'absolute', bottom: '0rem', right: '1.5rem', transform: 'scale(0.75)', margin: 0}}><RemoveIcon /></IconButton>
                <IconButton onClick={addClick} sx={{position: 'absolute', bottom: '0rem', right: '0', transform: 'scale(0.75)', margin: 0}}><AddIcon /></IconButton>
            </div>
        </div>
    )
}
